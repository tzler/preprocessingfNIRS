...
%
% OUTPUTS:
% data.SD - data structure describing the probe


function data = ty_PruneChannels(data) 
fprintf('\t\tworking on %s ... \n', mfilename)
% function SD = enPruneChannels(d,SD,tInc,dRange,SNRthresh,SDrange,resetFlag)

% load cfg and data into correct format
cfg = data.cfg; 
d = data.d; 
SD = data.SD; 
v2struct(cfg.pruneChan)
info.dRange = dRange; 
info.SNRthresh = SNRthresh; 
info.SDrange = SDrange; 
