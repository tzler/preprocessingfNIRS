...
function [data] = enStimRejection(data)

fprintf('\t\tworking on %s ... \n', mfilename)
t = data.t; info.t = t; 
s = data.s; info.s = s; 
if isfield(data.procResult,'tIncAuto')
    tIncAuto = data.procResult.tIncAuto; 
else 
    tIncAuto = []; 
end 

if isfield(data,'tIncMan') 
    tIncMan = data.tIncMan; 
else 
    tIncMan = ones(size(data.t))
end 
info.tIncMan = tIncMan; 

tRange = data.cfg.stimReject.tRange;
...
data.s = s; 
data = updateRecord(data, mfilename, info, 0); 
end
