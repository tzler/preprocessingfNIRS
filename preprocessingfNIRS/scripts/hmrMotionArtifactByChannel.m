...
function data = tytest_MotionArtifactByChannel(data)
fprintf('\t\tworking on %s ... \n', mfilename)

%        [tInc,tIncCh] = MotionArtifactByChannel(d, fs, SD, tIncMan, tMotion, tMask, std_thresh, amp_thresh)
cfg = data.cfg; 
v2struct(cfg.motionArt) 
d = data.procResult.dod; 
fs = data.t; 
fs = 1/(fs(2)-fs(1));
cfg.fs = fs; info.fs = fs; % calculate the sampling frequency in Hz from t
SD = data.SD; 
tIncMan = ones(size(d,1),1); 
info.tMotion = tMotion; 
info.tMask = tMask; 
info.std_thresh = std_thresh; 
info.amp_thresh = amp_thresh; 
...
for iCh = 1:length(lstAct)
    ...
    lstActTmp = find(SD.MeasList(:,1)==SD.MeasList(lstAct(iCh),1) & ...
        SD.MeasList(:,2)==SD.MeasList(lstAct(iCh),2) );
...
data.procResult.tIncAuto = tInc; 
data.procResult.tIncChAuto = tIncCh; 
results = 0; 
