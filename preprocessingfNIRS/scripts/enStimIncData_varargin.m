...
function [data] = enStimIncData_varargin(data)

fprintf('\t\tworking on %s ... \n', mfilename)
s = data.s; 
t = data.t; 
userdata = data.userdata; 
varInput = []; % AUTONIRS : changed all 'vargin's below to varInput

%function [s,nFuncParam,funcParam,funcParamFormat,funcParamVal] = enStimIncData_varargin(s,t,userdata,varInput)
...
for iParam=1:nFuncParam
    if iParam > length(varInput)
        varInput{iParam}.val = [0 0];	
    end
    k = findstr(userdata.cnames{iParam},' ');
    userdata.cnames{iParam}(k) = '_';
    varInput{iParam}.name = userdata.cnames{iParam};
end

for iParam = 1:nFuncParam
    for iParam0=1:length(varInput)
        if strcmp(varInput{iParam0}.name,userdata.cnames{iParam})
            funcParam{iParam} = varInput{iParam0}.name;
            funcParamFormat{iParam} = '%0.1f %0.1f';
            funcParamVal{iParam} = varInput{iParam0}.val;
            continue;
        end
    end
end
